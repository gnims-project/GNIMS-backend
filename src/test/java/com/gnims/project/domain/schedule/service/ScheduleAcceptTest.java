...
public class ScheduleAcceptTest {
    ...
    @DisplayName("이미 수락 혹은 거절된을 수락하려는 경우 " +
    ...
    @Disabled("지금은 너무 바빠요...")
    @Test
    void 일정_수락_실패_케이스4() throws Exception {

    }

    @DisplayName("내가 만든 일정을 수략하려는 경우 " +
            "상태 코드 400 발생" +
            "ScheduleStatus 필드는 ACCEPT로 유지된다.")
    @Test
    void 일정_수락_실패_케이스5() throws Exception {
        Long hostId = userRepository.findByNickname("딸기").get().getId();
    ...
    @DisplayName("일정에 초대받은 사용자가 일정 수락 시, 수락한 사용자의 Schedule 엔티티의" +
            "ScheduleStatus 필드 PENDING -> ACCEPT 로 변경되고" +
