...
public class SocialSignupTest {
    ...
    void 회원가입성공테스트() throws Exception {
        ...
        MockMultipartFile signupFile1 = new MockMultipartFile("data", "", "application/json", "{\"nickname\" : \"딸기\",\"username\": \"이땡땡\", \"email\": \"ddalgi@kakao.com\", \"socialCode\": \"KAKAO\"}".getBytes());
        MockMultipartFile signupFile2 = new MockMultipartFile("data", "", "application/json", "{\"nickname\" : \"햄버거\",\"username\": \"정땡땡\", \"email\": \"ddalgi@naver.com\", \"socialCode\": \"NAVER\"}".getBytes());
    ...
    @DisplayName("소셜가입 시 이메일 혹은 닉네임 중복 - 상태코드 400, 에러 메세지를 반환, db에 저장 실패")
    @Test
    void 회원가입실패테스트1() throws Exception {

        //중복 체크를 위해 db에 넣을 데이터
        MockMultipartFile successFile = new MockMultipartFile("data", "", "application/json", "{\"nickname\" : \"딸기\",\"username\": \"이땡땡\", \"email\": \"ddalgi@gmail.com\", \"socialCode\": \"KAKAO\"}".getBytes());

        //이메일 + 소셜 중복
        MockMultipartFile failFile1 = new MockMultipartFile("data", "", "application/json", "{\"nickname\" : \"김밥\",\"username\": \"박땡땡\", \"email\": \"ddalgi@gmail.com\", \"socialCode\": \"KAKAO\"}".getBytes());

        //닉네임 중복
        MockMultipartFile failFile2 = new MockMultipartFile("data", "", "application/json", "{\"nickname\" : \"딸기\",\"username\": \"김땡땡\", \"email\": \"ddalgi2@gmail.com\", \"socialCode\": \"NAVER\"}".getBytes());
    ...
    void 회원가입실패테스트2() throws Exception {
        ...
        //정규식 불일치
        MockMultipartFile failFile4 = new MockMultipartFile("data", "", "application/json", "{\"nickname\" : \"오렌지1234567890\",\"username\": \"신땡땡1\", \"email\": \"orange@kakaocom\", \"socialCode\": \"KAKAO\"}".getBytes());

        String expression1 = "$.[?(@.messages == ['%s', '%s', '%s'])]";
